...
export function AddNewsPanel({ defaultChannel, onCreated }: AddNewsPanelProps) {
  ...
  const [channel, setChannel] = useState<NewsChannel>(defaultChannel);
  const [teamScope, setTeamScope] = useState<TeamScope>("general");
  ...
  return (
    <section className="rounded-2xl border border-[#214C9B] bg-white p-4 shadow-sm sm:p-5">
      <h2 className="text-sm font-extrabold uppercase text-[#214C9B]">Nueva noticia</h2>
