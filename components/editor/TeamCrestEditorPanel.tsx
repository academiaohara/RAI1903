...
export function TeamCrestEditorPanel({ onClose }: TeamCrestEditorPanelProps) {
        ...
        <div className="grid max-h-48 grid-cols-3 gap-2 overflow-y-auto sm:grid-cols-4">
          {filteredCatalog.map((entry) => (
            <button
              ...
              className="flex min-h-16 flex-col items-center justify-center gap-1 rounded-lg border border-slate-100 p-2 hover:border-[#214C9B]/40 hover:bg-blue-50 active:bg-blue-100"
              title={entry.path}
            >
              <Image
                src={entry.path}
  ...
  const renderCrestActions = (teamId: string, assigned: string | undefined) => (
    <div className="flex shrink-0 flex-wrap gap-1">
      <button
        ...
        className="min-h-9 rounded-lg border border-[#214C9B]/20 px-2.5 py-1.5 text-[10px] font-extrabold uppercase text-[#214C9B] hover:bg-blue-50 active:bg-blue-100"
      >
        Elegir
      </button>
      {assigned && (
        <button
          ...
          className="min-h-9 rounded-lg border border-slate-200 px-2.5 py-1.5 text-[10px] font-bold text-slate-500 hover:bg-slate-50 active:bg-slate-100"
        >
          Quitar
        </button>
      )}
  ...
  return (
    <EditorPanelFrame
      title="Escudos"
      subtitle={`Temporada ${viewedSeason.label}`}
      onClose={onClose}
      busy={busy}
      message={message}
      footer={
        <button
          ...
          disabled={busy}
          onClick={() => void save()}
          className="min-h-11 w-full rounded-xl bg-[#214C9B] py-2.5 text-xs font-extrabold uppercase text-white hover:bg-[#173a78] active:bg-[#0f2d5c] disabled:opacity-50"
        >
            ...
            {femSlots.map((slot, index) => {
              ...
              return (
                <li
                  ...
                  className="flex flex-col gap-2 rounded-xl border border-[#981915]/15 bg-rose-50/30 px-2 py-2 sm:flex-row sm:items-start"
                >
              ...
              <ul className="space-y-2">
                {otherTeams.map((team) => {
                  ...
                  return (
                    <li
                      key={team.id}
                      className="flex flex-col gap-2 rounded-xl border border-slate-100 px-2 py-2 sm:flex-row sm:items-center"
                    >
