...
export function TransferMarketEditorPanel({ onClose }: TransferMarketEditorPanelProps) {
  ...
  return (
    <div className="flex max-h-[min(calc(100vh-7rem),40rem)] w-[min(100vw-2rem,28rem)] flex-col rounded-2xl border border-[#214C9B]/20 bg-white shadow-2xl">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-4 py-3">
        <p className="mb-3 text-xs leading-relaxed text-slate-600">
          Ventanas de mercado y movimientos del carrusel para <strong>{viewedSeason.label}</strong>. Edita cada ficha en
          el carrusel; aquí gestionas ventanas y altas nuevas.
        </p>
      ...
      <div className="shrink-0 border-t border-slate-100 px-4 py-3">
        {message && (
          <p className={`mb-2 text-xs font-bold ${messageIsError ? "text-[#981915]" : "text-emerald-700"}`}>
            {message}
          </p>
        )}
        <button
