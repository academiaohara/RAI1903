...
export function PlayerCareerTimeline({ player }: { player: SquadPlayer }) {
  ...
  return (
    <div className="relative space-y-0">
      <div className="absolute bottom-2 left-[1.15rem] top-2 w-px bg-gradient-to-b from-[#214C9B] via-[#214C9B]/30 to-transparent" />
      ...
      {career.map((entry, index) => (
        <motion.div
          ...
          className="relative grid gap-4 pb-6 pl-12 sm:grid-cols-[1fr_auto]"
        >
          <span className="absolute left-3 top-1.5 h-3.5 w-3.5 rounded-full border-2 border-white bg-[#214C9B] shadow-md shadow-blue-950/20" />
