...
export function SquadHeader({ club }: { club: SquadClubInfo }) {
  return (
    <motion.section
      ...
      className="relative overflow-hidden rounded-[2rem] border border-[#214C9B]/15 bg-gradient-to-br from-[#0f2347] via-[#173a78] to-[#214C9B] p-6 text-white shadow-[0_24px_60px_rgba(15,35,71,0.35)] sm:p-8"
    >
      ...
      <div className="relative flex flex-col gap-8 lg:flex-row lg:items-end lg:justify-between">
        <div className="flex flex-col gap-5 sm:flex-row sm:items-center">
          <div className="relative flex h-24 w-24 shrink-0 items-center justify-center rounded-3xl border border-white/20 bg-white/10 p-3 backdrop-blur-sm">
          ...
          <div>
            ...
            <h1 className="mt-1 text-3xl font-extrabold uppercase tracking-tight sm:text-4xl">{club.nombre}</h1>
        ...
        <div className="grid grid-cols-3 gap-2 sm:grid-cols-6 sm:gap-3">
          {statItems.map((item, index) => (
            ...
            >
              ...
              <p className="mt-1 text-2xl font-extrabold tabular-nums sm:text-3xl">{club.stats[item.key]}</p>
            </motion.div>
          ))}
        </div>
      </div>
