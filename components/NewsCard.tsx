...
export function NewsCard({ item, featured = false }: { item: NewsItem; featured?: boolean }) {
  return (
    <article className={`grid gap-4 rounded-3xl border border-[#214C9B]/30 bg-white p-4 shadow-[0_12px_30px_rgba(17,24,39,0.06)] transition hover:-translate-y-0.5 hover:border-[#214C9B] sm:grid-cols-[160px_1fr] ${featured ? "border-[#214C9B] bg-gradient-to-br from-white to-blue-50" : ""}`}>
      <div className="flex min-h-32 items-center justify-center rounded-2xl bg-[linear-gradient(135deg,#214C9B_0%,#214C9B_48%,#ffffff_48%,#ffffff_56%,#981915_56%,#981915_100%)] text-4xl font-extrabold text-white shadow-inner">
      ...
      <div>
        ...
        <h3 className={featured ? "text-3xl font-extrabold uppercase leading-tight text-[#214C9B]" : "text-xl font-extrabold uppercase leading-tight text-[#214C9B]"}>{item.title}</h3>
