...
export function MatchCard({ match, compact = false }: { match: Match; compact?: boolean }) {
  ...
  return (
    <article className="rounded-2xl border border-[#214C9B]/25 bg-white p-4 shadow-[0_10px_24px_rgba(17,24,39,0.05)] transition hover:border-[#214C9B]">
      <div className="mb-3 flex items-center justify-between gap-3">
        <Badge tone={match.status === "finished" ? "slate" : "blue"}>{match.status === "finished" ? "Finalizado" : "Programado"}</Badge>
        <span className="text-xs font-bold uppercase tracking-[0.08em] text-slate-500">J{match.matchday}</span>
      </div>
      <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-3">
        <p className={`text-sm font-extrabold ${avilesHome ? "text-[#214C9B]" : "text-slate-700"}`}>{match.homeTeam}</p>
        ...
        <p className={`text-right text-sm font-extrabold ${avilesAway ? "text-[#214C9B]" : "text-slate-700"}`}>{match.awayTeam}</p>
      </div>
      {!compact && (
