  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-[#c4121a]/20 bg-white">
      <table className="w-full text-left text-sm">
        <thead className="bg-[#c4121a] text-[11px] uppercase tracking-[0.18em] text-white">
          <tr>
        ...
        <tbody className="divide-y divide-slate-100">
          {rows.slice(0, compact ? 10 : rows.length).map((team) => {
            ...
            return (
              <tr key={team.id} className={team.id === RAI_TEAM_ID ? "bg-red-50 text-[#c4121a]" : "text-slate-700"}>
                <td className="px-3 py-3 font-black">{team.position}</td>
                <td className="px-3 py-3">
                  <div className="flex items-center gap-2">
                    <span className="flex h-8 w-8 items-center justify-center rounded-xl border border-[#c4121a]/20 bg-white text-[10px] font-black text-[#1c4f9c]">{team.crestInitials}</span>
                ...
                <td className="px-3 py-3 text-center text-base font-black text-[#c4121a]">{team.stats.points}</td>
                {!compact && (
                  <td className="px-3 py-3">
