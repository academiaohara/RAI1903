  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-[#214C9B]/20 bg-white">
      <table className="w-full text-left text-sm">
        <thead className="bg-[#214C9B] text-[11px] uppercase tracking-[0.08em] text-white">
          <tr>
              ...
              <tr key={team.id} className={team.id === RAI_TEAM_ID ? "bg-blue-50 text-[#214C9B]" : "text-slate-700"}>
                ...
                <td className="px-3 py-3">
                  <div className="flex items-center gap-2">
                    <span className="flex h-8 w-8 items-center justify-center rounded-xl border border-[#214C9B]/20 bg-white text-[10px] font-extrabold text-[#214C9B]">{team.crestInitials}</span>
                ...
                <td className="px-3 py-3 text-center text-base font-extrabold text-[#214C9B]">{team.stats.points}</td>
                {!compact && (
                  <td className="px-3 py-3">
                    <div className="flex gap-1">
                      {team.form.map((result, index) => (
                        <span key={`${team.id}-${result}-${index}`} className={`flex h-6 w-6 items-center justify-center rounded-full text-[10px] font-extrabold ${resultTone(result)}`}>
                          {result}
                        </span>
                      ))}
                    </div>
