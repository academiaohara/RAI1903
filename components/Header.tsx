    ...
    <header className="fixed inset-x-0 top-0 z-40 border-b-4 border-[#981915] bg-[#214C9B] text-white shadow-xl shadow-blue-950/15">
      <div className="mx-auto flex h-20 max-w-[1480px] items-center gap-4 px-4 sm:px-6 lg:px-8">
        <Link href="/" className="group flex items-center gap-3" onClick={() => setOpen(false)}>
          <span className="flex h-12 w-12 items-center justify-center rounded-full border-2 border-white bg-white text-[#214C9B] shadow-lg">
          ...
          {navItems.map((item) => {
            ...
            return (
              <div key={item.href} className="group relative">
                <Link href={item.href} style={active ? { color: "#214C9B" } : undefined} className={cn("inline-flex items-center gap-1 rounded-full px-4 py-2 text-sm font-bold uppercase tracking-normal transition hover:bg-white hover:text-[#214C9B]", active ? "bg-white shadow-lg shadow-blue-950/20" : "text-white")}>
                  {item.label}
            ...
            {navItems.map((item) => {
              ...
              return (
                <div key={item.href} className="rounded-2xl border border-white/10 bg-white/5 p-1">
                  <Link href={item.href} style={active ? { color: "#214C9B" } : undefined} onClick={() => setOpen(false)} className={cn("block rounded-xl px-4 py-3 text-sm font-bold uppercase tracking-normal", active ? "bg-white" : "text-white")}>
                    {item.label}
                  </Link>
                  {hasChildren && (
