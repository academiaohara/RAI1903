...
import { computeQuinielaRankingFromSupabase } from "@/lib/quiniela/compute-rankings";
...
export async function GET(request: Request) {
  ...
  if (scope === "round") {
    ...
    try {
      ...
      console.error("quiniela ranking round", message);
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : "Error al calcular el ranking";
    console.error("quiniela ranking season", message);
    return NextResponse.json({ error: message }, { status: 500 });
  }
}
