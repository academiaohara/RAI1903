...
import { computeQuinigolUserRoundFromSupabase } from "@/lib/juegos/compute-rankings";
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const result = await computeQuinigolUserRoundFromSupabase(
      supabase,
    ...
    const message = error instanceof Error ? error.message : "Error al cargar el quinigol";
    console.error("quinigol user-round", message);
