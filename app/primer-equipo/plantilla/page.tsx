...
export default function PlantillaPage() {
  ...
  return (
    <div className="space-y-6">
      <PageHero eyebrow="Primer Equipo" title="Plantilla" description="Jugadores, parte semanal y fichas individuales del Real Aviles Industrial." />
        ...
        {selected && (
          <div className="grid gap-6 lg:grid-cols-[0.45fr_1fr]">
            <div className="rounded-[2rem] border border-[#214C9B]/20 bg-gradient-to-br from-white via-blue-100 to-[#214C9B] p-6 text-center text-slate-950">
              <div className="flex aspect-[4/5] items-center justify-center rounded-[1.5rem] bg-[#214C9B] text-7xl font-extrabold text-white">{selected.firstName[0]}{selected.lastName[0]}</div>
            ...
            <div>
              ...
              <div className="mt-5 grid grid-cols-2 gap-3 md:grid-cols-3">
                {[["Partidos", selected.stats.appearances], ["Goles", selected.stats.goals], ["Asistencias", selected.stats.assists], ["Minutos", selected.stats.minutes], ["Amarillas", selected.stats.yellowCards], ["Rojas", selected.stats.redCards]].map(([label, value]) => <div key={label} className="rounded-2xl border border-[#214C9B]/20 bg-blue-50 p-4"><p className="text-xs uppercase tracking-normal text-slate-500">{label}</p><p className="mt-1 text-2xl font-extrabold text-[#214C9B]">{value}</p></div>)}
              </div>
            </div>
          </div>
        )}
