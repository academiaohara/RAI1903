  ...
  return (
    <div className="space-y-6">
      <section className="rounded-[2rem] border border-[#c4121a]/25 bg-white p-6 shadow-[0_18px_45px_rgba(17,24,39,0.08)]">
        <Badge tone="red">Competicion</Badge>
        <h1 className="mt-4 text-5xl font-black uppercase text-[#c4121a]">Liga y rivales</h1>
        <p className="mt-3 max-w-3xl text-slate-600">Tabla completa de equipos, datos basicos, estado de forma y detalle modal con plantilla, estadisticas y calendario.</p>
      </section>

      <Card eyebrow="Clasificacion" title="Tabla de liga"><LeagueTable teams={teams} /></Card>

      <Card eyebrow="Rivales" title="Todos los equipos">
        <div className="overflow-hidden rounded-2xl border border-[#c4121a]/20 bg-white">
          <table className="w-full min-w-[900px] text-left text-sm">
            <thead className="bg-[#c4121a] text-[11px] uppercase tracking-[0.18em] text-white"><tr><th className="px-4 py-3">Escudo</th><th className="px-4 py-3">Equipo</th><th className="px-4 py-3">Ciudad</th><th className="px-4 py-3">Estadio</th><th className="px-4 py-3">Entrenador</th><th className="px-4 py-3">Pos.</th><th className="px-4 py-3">Ultimos 5</th><th className="px-4 py-3">Detalle</th></tr></thead>
            <tbody className="divide-y divide-slate-100">
              {[...teams].sort((a, b) => a.position - b.position).map((team) => (
                <tr key={team.id} className="text-slate-700">
                  <td className="px-4 py-3"><span className="flex h-10 w-10 items-center justify-center rounded-2xl border border-[#c4121a]/20 bg-red-50 text-xs font-black text-[#c4121a]">{team.crestInitials}</span></td>
                  <td className="px-4 py-3 font-black text-slate-900">{team.name}</td>
                  <td className="px-4 py-3">{team.city}</td>
                  <td className="px-4 py-3">{team.stadium}</td>
                  <td className="px-4 py-3">{team.coach}</td>
                  <td className="px-4 py-3 font-black text-[#c4121a]">{team.position}</td>
                  ...
                  <td className="px-4 py-3"><button onClick={() => setSelected(team)} className="rounded-full bg-[#c4121a] px-4 py-2 text-xs font-black uppercase tracking-[0.14em] text-white transition hover:bg-[#8f0f14]">Ver</button></td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        {selected && (
          <div className="space-y-5">
            <div className="grid gap-4 md:grid-cols-4">
              {[['Ciudad', selected.city], ['Estadio', selected.stadium], ['Entrenador', selected.coach], ['Fundado', selected.founded]].map(([label, value]) => <div key={label} className="rounded-2xl border border-slate-200 bg-slate-50 p-4"><p className="text-xs uppercase tracking-[0.18em] text-slate-500">{label}</p><p className="mt-1 font-black text-slate-900">{value}</p></div>)}
            </div>
            <div className="grid gap-5 lg:grid-cols-2">
              <Card eyebrow="Plantilla basica" title="Jugadores referencia" dense>
                <div className="space-y-2">{(selected.id === RAI_TEAM_ID ? players.slice(0, 6) : players.slice(0, 5)).map((player) => <div key={player.id} className="flex items-center justify-between rounded-2xl border border-slate-200 bg-slate-50 p-3 text-sm"><span className="font-bold text-slate-900">{player.displayName}</span><span className="text-slate-500">{player.position}</span></div>)}</div>
              </Card>
              <Card eyebrow="Estadisticas" title="Balance general" dense>
                <div className="grid grid-cols-3 gap-3">{[['PJ', selected.stats.played], ['G', selected.stats.won], ['E', selected.stats.drawn], ['P', selected.stats.lost], ['GF', selected.stats.goalsFor], ['GC', selected.stats.goalsAgainst]].map(([label, value]) => <div key={label} className="rounded-2xl border border-[#c4121a]/20 bg-red-50 p-3 text-center"><p className="text-xs text-slate-500">{label}</p><p className="text-2xl font-black text-[#c4121a]">{value}</p></div>)}</div>
              </Card>
            </div>
